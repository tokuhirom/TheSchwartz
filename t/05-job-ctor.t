...
my $job1 = Enegger::Job->new_from_array("feedmajor", $fargs);
isa_ok($job1, 'Enegger::Job');
my $job2 = Enegger::Job->new_from_array("feedmajor", \$fargs);
isa_ok($job2, 'Enegger::Job');
my $job3 = Enegger::Job->new(funcname => 'feedmajor', arg => $args);
isa_ok($job3, 'Enegger::Job');
my $job4 = Enegger::Job->new(funcname => 'feedmajor', arg => $fargs);
isa_ok($job4, 'Enegger::Job');
my $job5 = Enegger::Job->new(funcname => 'feedmajor', arg => \$fargs);
isa_ok($job5, 'Enegger::Job');
...
isa_ok $job6, 'Enegger::Job';

# second arg can also be an arrayref
my $job_a1  = Enegger::Job->new_from_array("feedmajor", [ 'cheese', 'water', 'beer' ]);
my $job_a2  = Enegger::Job->new(funcname => "feedmajor",
                                    arg      => [ 'cheese', 'water', 'beer' ]);
is_deeply($job_a1->column_values, $job_a2->column_values, "ctors with arrayrefs match");

my $jobbad = eval { Enegger::Job->new(
                                          funcname => 'feeddog',
...
$jobbad = eval { Enegger::Job->new_from_array("feeddog", "scalar1", "scalar2") };
ok(!$jobbad, "no bad job");
ok($@,       "error creating job with bad argument");
...
$jobbad = eval { Enegger::Job->new_from_array("feeddog", { with => 'poison' }, { extra => 'arg' }); };
ok(!$jobbad, "no bad job");
ok($@,       "error creating job with bad argument");
